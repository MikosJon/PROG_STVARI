...
def obrni_vrstni_red(niz):
    return ' '.join(reversed(niz.split()))
# =====================================================================@020278=
# 2. podnaloga
...
def obrni_besede(niz):
    return ' '.join(beseda[::-1] for beseda in niz.split())
# =====================================================================@020279=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MjcxLCJwYXJ0IjoyMDI3N30:1jD4Rh:GJjY8nbIPtPmkLoGjr49fNM-QaY'
        try:
            Check.equal('obrni_vrstni_red("kdor drugemu jamo koplje ne pade daleč od drevesa")'
                        , "drevesa od daleč pade ne koplje jamo drugemu kdor")
            Check.equal('obrni_vrstni_red("a b c")', 'c b a')
            Check.equal('obrni_vrstni_red("brezpresledkov")', 'brezpresledkov')
            Check.secret(obrni_vrstni_red("sdf sdf sdfsdwag awgwaer sdfW SWEF"))
            Check.secret(obrni_vrstni_red("EZ SDFG JSDSs sgdwh"))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MjcxLCJwYXJ0IjoyMDI3OH0:1jD4Rh:QZ4rQnqroayaKDb_Tct0nWE793Q'
        try:
            Check.equal('obrni_besede("palindromom kot je kajak ta funkcija ne naredi nič")'
                        , "momordnilap tok ej kajak at ajicknuf en ideran čin")
            Check.equal('obrni_besede("abba kajak alisebomartanatramobesila")'
                        , "abba kajak alisebomartanatramobesila")
            Check.equal('obrni_besede("enobesednost")', 'tsondesebone')
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MjcxLCJwYXJ0IjoyMDI3OX0:1jD4Rh:fOOb80XCpHHwuMWL03xGrFYMq8I'
        try:
            Check.equal('sosed_naj_trpi("a bc def")', "f ed cba")
            Check.equal('sosed_naj_trpi("petunije so si zaslužile lepši konec")'
                        , 'cenokišp el el ižulsazis oseji nutep')
            Check.equal('sosed_naj_trpi("zbogom prečudovite petunije")'
                        , 'ejinut epetivoduče rpmogobz')
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
