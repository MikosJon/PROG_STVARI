...
def naivna_resitev(niz):
    navadni = niz.count('(') - niz.count(')')
    oglati = niz.count('[') - niz.count(']')
    zaviti = niz.count('{') - niz.count('}')
    return navadni == 0 and oglati == 0 and zaviti == 0
# =====================================================================@013394=
# 2. podnaloga
...
def gnezdeni_oklepaji(niz):
    ...
    for znak in niz:
        if znak in '([{':
            ...
            if len(stack) == 0 or stack[-1] != pripadajoc_oklepaj[znak]:
                return False
            else:
                stack.pop()
    return len(stack) == 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MjcxLCJwYXJ0IjoyMDI2OX0:1jD3jn:0BcYdwyfl8Lf11R8D-Tb01xAKmQ'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MjcxLCJwYXJ0IjoxMzM5NH0:1jD3jn:Z4N5pEd1XU7bEenFaEZ2S1aom5E'
        try:
            ...
            Check.equal('gnezdeni_oklepaji("(([{}]))")',True)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
